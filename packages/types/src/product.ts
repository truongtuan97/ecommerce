...
export const ProductFormSchema = z.object({
  name: z.
    string({ message: "Product name is required!" }).min(1, { message: "Product name is required!" }),
  shortDescription: z
  ...
  sizes: z.array(z.enum(sizes)).min(1, { message: "At leat one size is required!" }),
  colors: z.array(z.enum(colors)).min(1, { message: "At leat one color is required!" }),
  images: z.record(z.string(), z.string(), { message: "Image for each color is required!"}),
})
.refine((data) => {
  const missingImages = data.colors.filter(
    (color: stirng) => !data.images?.[color]
  );
